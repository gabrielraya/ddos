...
def denoising_fn(config, sde, shape, inverse_scaler, eps):
    """
    Create a denoising function. This functions is basically `get_sampling_fn` but with a given initial state

    :param x_init: the initial state at time t where we start denoising (solving the backward SDE)
